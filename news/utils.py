...
stop_words = {
    ...
    '출신', '새내기주', '명문', '기적', '초유', '연하', '완행', '지지자들', '여사', '주례',
    ...
    '합류', '업무', '전략', '남발', '방역', '대담', '위력', '과시하', '축하', '아이', '가동', '재가동', '치부', 
}
...
async def _get_gpt_response(prompt, temperature=0.7, max_tokens=300, split_sections=False):
    ...
    try:
        ...
        collector_prompt = {
            "role": "system",
            "content": """당신은 데이터 수집가입니다.
            1. 주요 키워드 추출 (정당명, 인명, 기관명 등)
            2. 언론사별 보도 빈도 분석
            3. 핵심 문장 식별"""
        }
